...
class ReplayBuffer {
  ...
  sample(k) {
    ...
    // Sample with replacement to guarantee termination even when randomness is
    // degenerate (e.g. tests that monkey-patch Math.random()).
    k = Math.min(k, this.buffer.length);
    const sampled = [];
    for (let i = 0; i < k; i++) {
      sampled.push(this.buffer[this._sampleIndex()]);
    }
    return sampled;
  }
