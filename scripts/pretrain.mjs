...
import { downloadCorpus } from './download-corpus.mjs';
...
if (isMain) {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === '--epochs') options.epochs = parseInt(args[++i]);
    else if (arg === '--batch') options.batchSize = parseInt(args[++i]);
    else if (arg === '--save-every') options.saveEvery = parseInt(args[++i]);
    else if (arg === '--max') options.maxSentences = parseInt(args[++i]);
    else if (arg === '--rl') options.rlPressure = parseFloat(args[++i]);
    else if (arg === '--min-len') options.minLen = parseInt(args[++i]);
    else if (arg === '--max-len') options.maxLen = parseInt(args[++i]);
    else if (arg === '--dedup') options.dedup = true;
    else if (arg === '--no-shuffle') options.shuffle = false;
    else if (arg === '--no-subsample') options.subsample = false;
    else if (arg === '--subsample-t') options.subsampleT = parseFloat(args[++i]);
  }
