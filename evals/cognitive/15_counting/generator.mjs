...
export class CountingGrammar {
  ...
  _init() {
    for (let p = 0; p < this.numPatterns; p++) {
      const threshold = this.minThreshold + 
        Math.floor(Math.random() * (this.maxThreshold - this.minThreshold + 1));
  ...
  generateTrainingData(count = 10000) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const countVariation = Math.floor(Math.random() * (this.maxThreshold + 2));
  ...
  generateTestData(count = 1000) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      lines.push(`${tokens.join(' ')}\t${expected}\t${testCount}\t${pattern.threshold}`);
    }
