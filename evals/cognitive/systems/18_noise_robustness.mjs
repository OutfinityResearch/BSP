...
export class NoiseRobustnessGrammar {
  ...
  addNoise(pattern, noiseLevel) {
    ...
    for (let i = 0; i < numCorruptions; i++) {
      const idx = Math.floor(Math.random() * noisy.length);
      const noiseType = Math.random();
      
      if (noiseType < 0.33) {
        // Replace with noise token
        noisy[idx] = this.noiseTokens[Math.floor(Math.random() * this.noiseTokens.length)];
  ...
  generateTestData(count = 1000) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      lines.push(`${noisy.join(' ')}\t${pattern.label}\t${noiseLevel}`);
    }
