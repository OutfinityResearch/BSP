...
export class ChunkingGrammar {
  ...
  _init() {
    ...
    for (let c = 0; c < this.numChunks; c++) {
      const size = this.minChunkSize + 
        Math.floor(Math.random() * (this.maxChunkSize - this.minChunkSize + 1));
      
      const tokens = [];
      for (let i = 0; i < size; i++) {
        tokens.push(`c${String(c).padStart(2, '0')}_${String(i).padStart(2, '0')}`);
      }
  ...
  generateSequence(numChunks = 3, includeFillers = true) {
    ...
    for (let i = 0; i < numChunks; i++) {
      // Maybe add filler
      if (includeFillers && Math.random() < 0.3) {
        const numFillers = 1 + Math.floor(Math.random() * 2);
        for (let f = 0; f < numFillers; f++) {
          sequence.push(this.fillers[Math.floor(Math.random() * this.fillers.length)]);
        }
      }
  ...
  generateTrainingData(count = 10000) {
    ...
    for (let i = 0; i < count; i++) {
      const numChunks = 2 + Math.floor(Math.random() * 4);
  ...
  generateTestData(count = 1000) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const cutPoint = 1 + Math.floor(Math.random() * (chunk.tokens.length - 1));
      const context = chunk.tokens.slice(0, cutPoint);
      const expected = chunk.tokens[cutPoint];
      
      lines.push(`${context.join(' ')}\t${expected}`);
    }
