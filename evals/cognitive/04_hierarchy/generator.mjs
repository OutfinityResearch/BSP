...
export class HierarchyGrammar {
  ...
  _init() {
    ...
    while (queue.length > 0) {
      ...
      const numChildren = 2 + Math.floor(Math.random() * this.branchingFactor);
  ...
  generateTrainingData(count = 10000) {
    ...
    for (let i = 0; i < count; i++) {
      // Random leaf
      const leaf = this.leaves[Math.floor(Math.random() * this.leaves.length)];
  ...
  generateTestData(count = 1000) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      lines.push(`${leaf}\t${ancestors[0]}\t${ancestors.join(',')}`);
    }
